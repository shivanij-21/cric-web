...
export class LoginComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.windowRef.recaptchaVerifier = new firebase.default.auth.RecaptchaVerifier("sign-in-button", {
      size: "invisible",
  ...
  Login(type:any) {
   ...
   this.isPendingLogin = true;
    ...
    if(this.EMAIL_REGEXP.test(this.f.userName.value)){
      this.recaptchaV3Service.execute('importantAction').subscribe((token: string) => {
        if (token != null || token != "") {
          this.authService.SignInWithEmail(data,type).then(()=>{
      ...
      this.recaptchaV3Service.execute('importantAction').subscribe((token: string) => {
        if (token != null || token != '') {
          let userData = {
            ...
            phoneNumber: this.SignUpForm.controls['phoneno'].value,
            address: "",
  ...
  //   this.authService.SignUp(this.EmailForm.controls['email'].value, this.EmailForm.controls['password'].value).then(() => {
  ...
  //   );
  // }
  loginWithGoogle(type: any) {
    this.authService.GoogleAuth(type).then(() => {
      this.closeRegisterPop();
    }
    );
  }

  loginWithFacebook(type: any) {
    this.authService.FacebookAuth(type);
  }
  ...
  loginWithPhone() {
    ...
    $('#page_loading').css('display', 'block');
    this.authService.otpSubmit(this.pf.otp.value).then(() => {
  ...
  getOtp() {
    if (this.pf.phoneNumber == null) {
      ...
      $('#phone_loading').css('display', 'block');
      this.phoneError = "";
  ...
  private loginViaTelegram(loginData: any) {
    this.ngZone.run(() => {
      ...
      if (loginData) {
        let userData = {
          firstName: loginData.first_name != null ? loginData.first_name : "",
          lastName: loginData.last_name != null ? loginData.last_name : "",
        ...
        this.loginService.register(userData).subscribe((resp: any) => {
          ...
          if (resp.errorCode === 0) {
            ...
            if(this.siteName=='dreamcric' && this.tokenService.getClickID()!='undefined'){
              let affData = {
  ...
  async loginWithMetamask(type: any) {
      ...
      this.loginService.getNonce(data).subscribe(async (resp: any) => {
        ...
        if (resp.status === 'Success') {
          let result = await ethereum.request({
            ...
            params: [
              `0x${this.toHex(`${environment.siteName}: ${resp.data.nonce}`)}`,
              resp.data.walletAdd,
            ],
          }).catch((error: any) => {
  ...
  getlanguages() {
    this.shareService._lagugageSub$.subscribe(data => {
      ...
      if (this.Update?.Signup == "Inscrever-se" || this.Update?.Signup == "Giriş yapmak") {
        $(".btn-signup").css('font-size', '10px');
      } else {
        $(".btn-signup").css('font-size', 'inherit');
      }
